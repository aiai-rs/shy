...
const ALLOWED_BOT_USERS = (process.env.ALLOWED_BOT_USERS || '').split(',').map(id => Number(id.trim())).filter(id => !isNaN(id));

if (process.env.VAPID_PUBLIC_KEY && process.env.VAPID_PRIVATE_KEY) {
    try {
        webpush.setVapidDetails(process.env.VAPID_EMAIL || 'mailto:admin@huiying.com', process.env.VAPID_PUBLIC_KEY, process.env.VAPID_PRIVATE_KEY);
    } catch (error) { console.error("Web Push Config Error:", error.message); }
}
...
const isCambodiaWorkingTime = () => { const h = (new Date().getUTCHours() + 7) % 24; return h >= 13 && h < 23; };
...
const forceDisconnectUser = async (targetId) => {
    try {
        ...
        if (sockets.length > 0) { sockets.forEach(s => { s.emit('force_disconnect'); s.disconnect(true); }); }
        onlineUsers.delete(targetId);
        io.to('admin_room').emit('user_status_change', { userId: targetId, online: false });
    } catch (e) { console.error(`Disconnect error ${targetId}:`, e); }
};

const notifyAdminUpdate = () => { io.emit('admin_update', { timestamp: Date.now() }); };
const getSetting = async (key) => { const res = await pool.query('SELECT value FROM settings WHERE key = $1', [key]); return res.rows.length > 0 ? res.rows[0].value : null; };
const setSetting = async (key, value) => { await pool.query('INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = $2', [key, value.toString()]); };
...
const logBalance = async (client, userId, type, amount, remark) => {
    ...
    await client.query("INSERT INTO balance_logs (user_id, type, amount, remark, balance_after) VALUES ($1, $2, $3, $4, $5)", [userId, type, amount, remark, currentBal]);
};
    ...
    try {
        ...
        if (inviterId) {
            ...
            if (bonus > 0) {
                ...
                io.to(notifySid).emit('new_message', { session_id: notifySid, sender: 'admin', content: content, msg_type: 'text', created_at: msgRes.rows[0].created_at });
...
function saveAuth() {
    try {
        const data = { users: Object.fromEntries(authorizedUsers), tokens: Object.fromEntries(groupTokens), configs: Object.fromEntries(groupConfigs) };
...
function factoryReset() {
    authorizedUsers.clear(); groupTokens.clear(); groupConfigs.clear();
    warningMessages.clear(); unauthorizedMessages.clear(); zlMessages.clear();
    for(let k in tpSessions) delete tpSessions[k];
    pendingAgentAuth.clear(); pendingPayouts.clear(); activePayoutMessages.clear(); 
    ...
    jsonData.forEach(row => {
        if(Array.isArray(row)) {
            ...
            if(row[6]) {
                ...
                if(!isNaN(ts)) {
                    if(ts > maxTimestamp) { maxTimestamp = ts; lastVisitDate = timeStr; }
                }
            }
        }
    });
    
    majorKeywords.forEach(keyword => { if (new RegExp(keyword, 'i').test(fullText)) detectedIssues.push(keyword); });
    detectedIssues = [...new Set(detectedIssues)];

    let summaryText = `🧾 重点筛查（忽略普通症状）\n\n`;
    if (detectedIssues.length > 0) summaryText += `🚨 检测到关键疾病记录：\n${detectedIssues.join('、')}\n`;
    else summaryText += `✅ 未检测到重大疾病关键词\n（已自动过滤感冒/发热/咳嗽等普通症状）\n`;

    if(lastVisitDate) summaryText += `\n📅 最后一次看病时间：${lastVisitDate}\n`;
    else summaryText += `\n📅 最后一次看病时间：未检测到有效日期\n`;
    summaryText += `\n⚠️ 注意：此分析仅基于文本。`;
    return summaryText;
}
...
// 替換 node-telegram-bot-api 的輔助函數
...
bot.command('ck', async (ctx) => {
    ...
    if (chatId === ALLOWED_GROUP_ID) {
        try {
            const userCount = await prisma.user.count(); const msgCount = await prisma.message.count(); const subCount = await prisma.pushSubscription.count();
            const users = await prisma.user.findMany({ take: 10, orderBy: { updatedAt: 'desc' }, include: { _count: { select: { messages: true } } } });
            let text = `📊 **系统状态统计**\n👥 总用户数: ${userCount}\n📡 推送订阅: ${subCount}\n💬 总消息数: ${msgCount}\n\n📝 **最近活跃 (Top 10):**\n`;
            const buttons = [];
            users.forEach(u => { text += `🆔 \`${u.id}\` | 👤 ${u.bossId || '无'} | 💬 ${u._count.messages}\n`; buttons.push([Markup.button.callback(`🗑 删除 ${u.id}`, `del_${u.id}`)]); });
            buttons.push([Markup.button.callback('❌ 关闭列表', 'cancel')]);
            return ctx.reply(text, { parse_mode: 'Markdown', ...Markup.inlineKeyboard(buttons) });
        } catch (e) { return ctx.reply("❌ 查询失败"); }
    }
    if (chatId === TG_ADMIN_GROUP_ID) {
        try {
            const [uRes, oRes, pRes] = await Promise.all([ pool.query('SELECT COUNT(*) FROM users'), pool.query('SELECT COUNT(*) FROM orders'), pool.query('SELECT COUNT(*) FROM products') ]);
            const uptime = process.uptime(); const runTimeStr = `${Math.floor(uptime / 86400)}天 ${Math.floor((uptime % 86400) / 3600)}小时 ${Math.floor((uptime % 3600) / 60)}分`;
            ...
            try {
                ...
                const reads = hitRes.rows[0].reads || 0; const hits = hitRes.rows[0].hits || 0;
            ...
            try {
                ...
                if (cloudRes?.credits) {
                    const cUsed = cloudRes.credits.usage.toFixed(2); const cPercent = cloudRes.credits.used_percent.toFixed(1);
                    const filled = Math.min(10, Math.round(cPercent / 10)); cloudBar = `\n${'■'.repeat(filled) + '□'.repeat(10 - filled)}`;
            ...
            const drawBar = (percent) => { const filled = Math.min(10, Math.max(0, Math.round(percent / 10))); return '■'.repeat(filled) + '□'.repeat(10 - filled); };
        ...
        } catch (err) { await ctx.reply(`❌ 执行失败：${err.message}`); }
    }
});
...
bot.command('qc', async (ctx) => {
    ...
    if (GROUP_CHAT_IDS.includes(ctx.chat.id) && await isAdmin(ctx.chat.id, ctx.from.id)) {
        return ctx.reply(t(ctx.chat.id, 'qc_confirm'), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: t(ctx.chat.id, 'btn_confirm'), callback_data: 'qc_yes' }], [{ text: t(ctx.chat.id, 'btn_cancel'), callback_data: 'qc_no' }]] }});
    }
});
...
async function handleLinkCommand(ctx, type) {
    ...
    const replyMsg = await ctx.reply(`${t(chatId, 'zl_msg')}\n\n${type === 'zl' ? t(chatId, 'zl_btn_title') : t(chatId, 'zj_btn_title')}`, {
        reply_markup: { inline_keyboard: [[{ text: '租车', callback_data: `${type}_租车` }, { text: '大飞', callback_data: `${type}_大飞` }], [{ text: '走药', callback_data: `${type}_走药` }, { text: '背债', callback_data: `${type}_背债` }]] }
    });
    zlMessages.set(replyMsg.message_id, { commandType: type, targetFirstName: ctx.message.reply_to_message?.from.first_name || '未知', targetUserId: ctx.message.reply_to_message?.from.id || 0 });
}
...
bot.command('lh', async (ctx) => {
    ...
    if (!await isAdmin(ctx.chat.id, ctx.from.id)) return ctx.reply(t(ctx.chat.id, 'perm_deny'));
    if (!ctx.message.reply_to_message) return;
    try { await bot.telegram.banChatMember(ctx.chat.id, ctx.message.reply_to_message.from.id); ctx.reply(t(ctx.chat.id, 'ban_msg')); } catch(e){}
});
...
bot.command('tp', async (ctx) => {
    ...
    if (!replyMsg || !replyMsg.document || !replyMsg.document.file_name.endsWith('.xlsx')) return ctx.reply("❌ 请回复 .xlsx 格式的 Excel 文件。");
    ...
    try {
        ...
        tpSessions[ctx.from.id] = { rawData: jsonData, mode: 'short', fileName: replyMsg.document.file_name.replace('.xlsx', ''), msgId: null };
        ...
        const previewMsg = await ctx.reply(`📄 ${tpSessions[ctx.from.id].fileName}的医疗文件预览（第 1 页 / 共 ${totalPages} 页）\n\n<pre>${page1}</pre>\n\n `, {
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: [[{ text: '⬅️ 上一页', callback_data: 'tp_prev_1' }, { text: '下一页 ➡️', callback_data: 'tp_next_1' }], [{ text: '🔘 显示/隐藏完整医院名称', callback_data: 'tp_toggle_mode_1' }], [{ text: '🗑️ 删除预览会话', callback_data: 'tp_delete_session' }]] }
        });
        tpSessions[ctx.from.id].msgId = previewMsg.message_id;
        await ctx.reply(generateMedicalSummary(jsonData));
    } catch (err) { ctx.reply("❌ 解析失败，请重试。"); }
});
...
bot.on('text', async (ctx, next) => {
    ...
    if (chatId === TG_ADMIN_GROUP_ID) {
        if (text === '/fix_db') {
            try {
                await pool.query('ALTER TABLE orders ADD COLUMN IF NOT EXISTS wallet TEXT;'); await pool.query('ALTER TABLE chats ADD COLUMN IF NOT EXISTS msg_type TEXT;'); await pool.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS source TEXT;'); await pool.query('ALTER TABLE orders ADD COLUMN IF NOT EXISTS source TEXT;'); await pool.query('ALTER TABLE chats ADD COLUMN IF NOT EXISTS source TEXT;');
...
bot.on('photo', async (ctx, next) => {
    const userId = ctx.from.id; const msg = ctx.message;
    ...
    if (msg.reply_to_message && activePayoutMessages.has(msg.reply_to_message.message_id)) {
        ...
        } catch (e) { await ctx.reply("❌ 发送失败，可能是用户已屏蔽机器人。"); }
    ...
    if (pendingPayouts.has(userId)) {
        ...
        const sentMsg = await bot.telegram.sendPhoto(BACKUP_GROUP_ID, ctx.message.photo[ctx.message.photo.length - 1].file_id, { caption: caption, parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: "❌ 财务驳回 (拒绝打款)", callback_data: `reject_pay_btn` }]] } });
        activePayoutMessages.set(sentMsg.message_id, { targetChatId: payoutInfo.chatId, targetUserId: userId, amount: payoutInfo.amount, operatorId: payoutInfo.adminId, operatorName: payoutInfo.adminName, targetUser: payoutInfo.targetUser });
...
bot.on('callback_query', async (ctx) => {
    ...
    if (data.startsWith('del_')) {
        ...
        try {
            await prisma.message.deleteMany({ where: { userId: targetId } }); await prisma.user.delete({ where: { id: targetId } });
            await forceDisconnectUser(targetId); io.emit('admin_user_deleted', targetId);
            await ctx.answerCbQuery(`已删除 ${targetId}`); return ctx.reply(`🗑 用户 \`${targetId}\` 数据已销毁。`, { parse_mode: 'Markdown' });
        } catch (e) { return ctx.answerCbQuery("失败"); }
    }
    if (data === 'confirm_clear_all') {
        try {
            await prisma.pushSubscription.deleteMany({}); await prisma.message.deleteMany({}); await prisma.user.deleteMany({});
            io.emit('admin_db_cleared'); io.emit('force_logout_all');
            const sockets = await io.fetchSockets(); sockets.forEach(s => s.disconnect(true)); onlineUsers.clear();
            return ctx.editMessageText("💥 **数据库已彻底格式化**");
        } catch (e) { return ctx.editMessageText(`❌ Error: ${e.message}`); }
    }
    if (data === 'cancel') { try { await ctx.deleteMessage(); } catch(e){} return; }

    // --- 商城網站機器人 ---
    if (data === 'qc_transactions') {
        await pool.query('TRUNCATE orders, withdrawals'); return ctx.editMessageText("🧹 <b>交易数据（订单、提现）已清空！</b>\n用户和聊天记录保留。", { parse_mode: 'HTML' });
    }
    if (data === 'qc_everything') {
        await pool.query('TRUNCATE users, orders, products, hiring, chats, withdrawals, settings'); return ctx.editMessageText("💥 <b>数据库已完全重置！</b>\n所有数据已永久删除。", { parse_mode: 'HTML' });
    }
    if (data === 'qc_cancel') { return ctx.editMessageText("✅ 操作已取消"); }

    if (data.startsWith('wd_confirm_')) {
        const parts = data.split('_'); const wdId = parts[2]; const userId = parts[3];
        await pool.query("UPDATE withdrawals SET status = '已完成' WHERE id = $1", [wdId]);
        const notifySid = `user_${userId}`; const content = '✅ 您的提现已处理，请查收。';
    ...
    if (data.startsWith('wd_reject_')) {
        const parts = data.split('_'); const wdId = parts[2]; const userId = parts[3]; const amount = parseFloat(parts[4]);
        ...
        const notifySid = `user_${userId}`; const content = '❌ 您的提现已被驳回，资金已退回余额。';
        const resDb = await pool.query("INSERT INTO chats (session_id, sender, content, msg_type) VALUES ($1, 'admin', $2, 'text') RETURNING created_at", [notifySid, content]);
        io.to(notifySid).emit('new_message', { session_id: notifySid, sender: 'admin', content: content, msg_type: 'text', created_at: resDb.rows[0].created_at });
        io.to(notifySid).emit('order_update');
    ...
    if (data.startsWith('pay_confirm_')) {
        const parts = data.split('_'); const orderId = parts[2]; const userId = parts[3];
        const orderRes = await pool.query("SELECT * FROM orders WHERE order_id = $1", [orderId]); const order = orderRes.rows[0];
        if (order && order.status !== '已支付') {
            await pool.query("UPDATE orders SET status = '已支付' WHERE order_id = $1", [orderId]);
            if (order.product_name === '余额充值') await pool.query("UPDATE users SET balance = balance + $1 WHERE id = $2", [parseFloat(order.usdt_amount), userId]);
            else try { await handleReferralBonus(userId, parseFloat(order.usdt_amount), '消费'); } catch (e) { }
            const notifySid = `user_${userId}`; const content = '✅ 您的支付已确认，订单正在处理中。';
            const resDb = await pool.query("INSERT INTO chats (session_id, sender, content, msg_type) VALUES ($1, 'admin', $2, 'text') RETURNING created_at", [notifySid, content]);
            io.to(notifySid).emit('new_message', { session_id: notifySid, sender: 'admin', content: content, msg_type: 'text', created_at: resDb.rows[0].created_at });
            io.to(notifySid).emit('order_update');
    ...
    if (['set_lang_cn', 'set_lang_tw'].includes(data)) {
        const lang = data === 'set_lang_cn' ? 'zh-CN' : 'zh-TW'; groupConfigs.set(chatId, { lang: lang }); saveAuth();
        try { await ctx.answerCbQuery(lang === 'zh-CN' ? '已设置为简体中文' : '已設置為繁體中文'); await ctx.deleteMessage(); } catch(e){}
        return ctx.reply(t(chatId, '请选择你的出行方式！'), { reply_markup: { inline_keyboard: [[{ text: t(chatId, 'btn_land'), callback_data: 'travel_land' }], [{ text: t(chatId, 'btn_flight'), callback_data: 'travel_flight' }]] } });
    }
    ...
    if (data === 'reject_pay_btn') {
        ...
        if (!activePayoutMessages.has(msg.message_id)) { await ctx.editMessageCaption(msg.caption + "\n\n⚠️ <b>此订单已失效或已被处理</b>", { parse_mode: 'HTML' }); return ctx.answerCbQuery("⚠️ 订单不存在"); }
        const payoutData = activePayoutMessages.get(msg.message_id);
        try { await bot.telegram.sendMessage(payoutData.targetChatId, `❌ <b>打款申请被驳回</b>\n\n你的打款申请（金额：${payoutData.amount}）已被财务驳回。\n⚠️如有疑问，请联系负责人。`, { parse_mode: 'HTML' }); } catch (e) { }
        try { await ctx.editMessageCaption(msg.caption + `\n\n❌ <b>已被 ${operatorName} 驳回</b>`, { parse_mode: 'HTML' }); } catch (e) { }
        activePayoutMessages.delete(msg.message_id); return ctx.answerCbQuery("✅ 已执行驳回操作");
    }
    if (data.startsWith('cancel_pay_')) {
        if (!await isAdmin(ctx.chat.id, ctx.from.id)) return ctx.answerCbQuery("❌ 无权限", { show_alert: true });
        const targetUserId = parseInt(data.split('_')[2]); let found = false;
        if (pendingPayouts.has(targetUserId)) { pendingPayouts.delete(targetUserId); found = true; }
        for (const [msgId, payoutData] of activePayoutMessages.entries()) {
            if (payoutData.targetUserId === targetUserId) {
                ...
                try { await bot.telegram.editMessageCaption(BACKUP_GROUP_ID, msgId, null, originalCaption + `\n\n⚠️ 此打款已被 <a href="tg://user?id=${ctx.from.id}">${ctx.from.first_name}</a> 取消！`, { parse_mode: 'HTML' }); } catch (e) { }
                activePayoutMessages.delete(msgId); found = true; break; 
            }
        }
        if (found) { await ctx.reply(`❌ 本次打款流程已取消\n操作人：<a href="tg://user?id=${ctx.from.id}">${ctx.from.first_name}</a>`, { parse_mode: 'HTML' }); return ctx.answerCbQuery("✅ 已取消"); }
        else return ctx.answerCbQuery("⚠️ 流程不存在或已结束", { show_alert: true });
    }
    if (data === 'qc_yes') {
        if (!await isAdmin(ctx.chat.id, ctx.from.id)) return;
        ...
        (async () => {
            ...
            while (i <= 1000 && consecutiveFails < 20) {
                try { await new Promise(r => setTimeout(r, 35)); await bot.telegram.deleteMessage(chatId, msg.message_id - i); consecutiveFails = 0; } catch (e) { consecutiveFails++; if (e.description && e.description.includes('message can\'t be deleted')) break; }
                i++;
            }
            try { await bot.telegram.editMessageText(chatId, msg.message_id, null, t(chatId, 'qc_done')); } catch (e) { try { await bot.telegram.sendMessage(chatId, t(chatId, 'qc_done')); } catch(e){} }
        })();
        return;
    }
    if (data === 'qc_no') { return ctx.editMessageText(t(ctx.chat.id, 'qc_cancel')); }
    
    // Excel 預覽分頁邏輯
    if (data.startsWith('tp_')) {
        const action = data.split('_')[1]; let currentPage = parseInt(data.split('_')[2]);
        let targetSession = null;
        for (const [uid, session] of Object.entries(tpSessions)) { if (session.msgId === msg.message_id) { targetSession = session; break; } }
        ...
        if (action === 'delete') {
            ...
            for (const [uid, session] of Object.entries(tpSessions)) { if (session.msgId === msg.message_id) { delete tpSessions[uid]; break; } }
            try { await ctx.deleteMessage(); } catch(e) {}
            await ctx.reply("🗑️ 文件预览已删除"); return ctx.answerCbQuery();
        }

        let newPage = currentPage; const totalPages = Math.ceil(targetSession.rawData.length / 8); 
        if (action === 'toggle') targetSession.mode = targetSession.mode === 'short' ? 'full' : 'short';
        else {
            newPage = action === 'prev' ? currentPage - 1 : currentPage + 1;
            if (newPage < 1) newPage = 1; if (newPage > totalPages) newPage = totalPages;
        ...
        try {
            await ctx.editMessageText(`📄 ${targetSession.fileName}的医疗文件预览（第 ${newPage} 页 / 共 ${totalPages} 页）\n\n<pre>${content}</pre>\n\n`, {
                parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: '⬅️ 上一页', callback_data: `tp_prev_${newPage}` }, { text: '下一页 ➡️', callback_data: `tp_next_${newPage}` }], [{ text: targetSession.mode === 'short' ? '🔘 显示完整医院名称' : '🔘 隐藏完整医院名称', callback_data: `tp_toggle_${newPage}` }], [{ text: '🗑️ 删除预览会话', callback_data: 'tp_delete_session' }]] }
            });
        } catch(e) {}
        return ctx.answerCbQuery();
    }
...
const adminAuth = (req, res, next) => { if(req.headers['authorization'] === ADMIN_TOKEN) next(); else res.status(401).json({msg:'Unauthorized'}); };
...
app.post('/api/user/login', loginLimiter, async (req, res) => {
    ...
    try {
        ...
        if(resDb.rows.length > 0) {
            if (await bcrypt.compare(password, resDb.rows[0].password)) res.json({ success: true, userId: resDb.rows[0].id, uid: resDb.rows[0].id, balance: parseFloat(resDb.rows[0].balance), inviteCode: resDb.rows[0].invite_code });
            else res.json({ success: false, msg: '账号或密码错误' });
        } else res.json({ success: false, msg: '账号或密码错误' });
...
app.post('/upload', async (req, res) => {
    try {
        ...
        if (GROUP_CHAT_IDS.includes(Number(chatid))) await sendToChat(Number(chatid), photoBuffer, caption, lat, lng);
...
// --- 客服系統 API ---
app.post('/api/user/check', async (req, res) => { try { const user = await prisma.user.findUnique({ where: { id: req.body.userId } }); res.json({ exists: !!user }); } catch (e) { res.json({ exists: false }); } });
app.get('/api/vapid-key', (req, res) => res.json({ publicKey: process.env.VAPID_PUBLIC_KEY }));
app.post('/api/subscribe', async (req, res) => {
    const { userId, subscription } = req.body; if (!userId || !subscription?.endpoint) return res.status(400).json({});
    try { await prisma.pushSubscription.upsert({ where: { endpoint: subscription.endpoint }, update: { userId, keys: subscription.keys }, create: { userId, endpoint: subscription.endpoint, keys: subscription.keys } }); res.status(201).json({ success: true }); } catch (e) { res.status(500).json({}); }
});
app.get('/api/history/:userId', async (req, res) => { try { const msgs = await prisma.message.findMany({ where: { userId: req.params.userId }, orderBy: { createdAt: 'asc' } }); res.json(msgs); } catch(e) { res.json([]); } });
app.get('/api/admin/users', async (req, res) => {
    try {
        const users = await prisma.user.findMany({ where: { isBlocked: false }, orderBy: { updatedAt: 'desc' }, include: { messages: { take: 1, orderBy: { createdAt: 'desc' } }, _count: { select: { messages: { where: { isFromUser: true, status: 'sent' } } } } } });
        res.json(users.map(u => ({ id: u.id, bossId: u.bossId, updatedAt: u.updatedAt, messages: u.messages, unreadCount: u._count.messages, isBlocked: u.isBlocked, isMuted: u.isMuted })));
    } catch (e) { res.status(500).json([]); }
});

// --- 商城系統 API ---
app.post('/api/notify-restock', async (req, res) => { sendTgNotify(`📢 <b>缺货补货提醒</b>\n商品: ${req.body.productName}\n客户联系: ${req.body.contact}\n请尽快补货！`); res.json({success: true}); });
...
app.post('/api/user/register', async (req, res) => {
    ...
    try {
        ...
        const id = uid || Math.floor(100000 + Math.random() * 900000); const hashedPassword = await bcrypt.hash(password, 10); const myInviteCode = Math.random().toString(36).substring(2, 6).toUpperCase();
        let inviterId = null;
        if (inviteCode) { const inviterRes = await pool.query('SELECT id FROM users WHERE invite_code = $1', [inviteCode.toUpperCase().trim()]); if (inviterRes.rows.length > 0) inviterId = inviterRes.rows[0].id; }
        ...
        notifyAdminUpdate(); res.json({ success: true, isNew: true, userId: id, uid: id, balance: 0, inviteCode: myInviteCode });
    } catch(e) { res.json({success:false, msg: e.message}); }
});
app.delete('/api/admin/user/:id', adminAuth, async (req, res) => {
    try {
        const uid = req.params.id; await pool.query('DELETE FROM users WHERE id = $1', [uid]); await pool.query('DELETE FROM orders WHERE user_id = $1', [uid]); await pool.query('DELETE FROM withdrawals WHERE user_id = $1', [uid]); await pool.query('DELETE FROM chats WHERE session_id = $1', [`user_${uid}`]);
        io.to(`user_${uid}`).emit('force_logout'); res.json({success: true});
    } catch(e) { res.status(500).json({success: false, msg: e.message}); }
});
app.get('/api/user/balance', async (req, res) => {
    try { const resDb = await pool.query('SELECT balance FROM users WHERE id = $1', [req.query.userId]); res.json({ success: resDb.rows.length > 0, balance: resDb.rows.length > 0 ? parseFloat(resDb.rows[0].balance) : 0 }); } catch(e) { res.json({success:false}); }
});
app.post('/api/user/change-password', async (req, res) => {
    try {
        ...
        await pool.query('UPDATE users SET password = $1 WHERE id = $2', [req.body.newPassword, req.body.userId]); res.json({success: true, msg: '修改成功'});
...
app.get('/api/user/records', async (req, res) => {
    try {
        if (req.query.type === 'withdraw') res.json((await pool.query('SELECT * FROM withdrawals WHERE user_id = $1 ORDER BY created_at DESC', [req.query.userId])).rows);
        else if (req.query.type === 'recharge') res.json((await pool.query("SELECT * FROM orders WHERE user_id = $1 AND product_name = '余额充值' ORDER BY created_at DESC", [req.query.userId])).rows);
        else res.json([]);
    } catch(e) { res.json([]); }
});
app.get('/api/user/balance_logs', async (req, res) => { try { res.json((await pool.query('SELECT * FROM balance_logs WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50', [req.query.userId])).rows); } catch(e) { res.status(500).json([]); } });
app.post('/api/order/confirm-payment', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) return res.json({success:false, msg:'请选择图片'});
        try { await bot.telegram.sendPhoto(TG_ADMIN_GROUP_ID, { source: req.file.buffer }, { caption: `📸 <b>收到支付凭证</b>\n单号: <code>${req.body.orderId}</code>\n用户ID: ${req.body.userId}\n请核对金额后在后台确认。`, parse_mode: 'HTML', reply_markup: { inline_keyboard: [[ { text: "✅ 已收到", callback_data: `pay_confirm_${req.body.orderId}_${req.body.userId}` }, { text: "❌ 未收到", callback_data: `pay_reject_${req.body.orderId}_${req.body.userId}` } ]] } }); } catch (e) { }
...
app.post('/api/withdraw', upload.single('file'), async (req, res) => {
    try {
        ...
        const options = { caption: `💸 <b>新提现申请 (${req.body.method})</b>\n用户: ${user.contact} (ID: ${req.body.userId})\n金额: ${amount} USDT\n账号: ${req.body.address || '无账号信息'}\nID: ${withdrawId}`, parse_mode: 'HTML', reply_markup: { inline_keyboard: [[ { text: "✅ 已打款", callback_data: `wd_confirm_${withdrawId}_${req.body.userId}` }, { text: "❌ 驳回", callback_data: `wd_reject_${withdrawId}_${req.body.userId}_${amount}` } ]] } };
        if (req.file) await bot.telegram.sendPhoto(TG_ADMIN_GROUP_ID, { source: req.file.buffer }, options); else await bot.telegram.sendMessage(TG_ADMIN_GROUP_ID, options.caption, options);
        notifyAdminUpdate(); res.json({ success: true });
...
app.get('/api/chat/history/:sid', async (req, res) => { try { res.json((await pool.query('SELECT * FROM chats WHERE session_id = $1 ORDER BY created_at ASC', [req.params.sid])).rows); } catch(e) { res.json([]); } });
...
app.post('/api/admin/user/balance', adminAuth, async (req, res) => {
    try {
        const val = parseFloat(req.body.amount); let sql = '';
        ...
        io.to(`user_${req.body.userId}`).emit('order_update'); res.json({success:true});
    } catch(e) { res.json({success:false}); }
});
...
app.post('/api/admin/order/ship', adminAuth, async (req, res) => {
    try {
        ...
        sendTgNotify(`🚚 <b>订单已发货</b>\n单号: <code>${req.body.orderId}</code>\n物流: ${req.body.trackingNumber}`); res.json({success:true});
    } catch(e) { res.status(500).json({success:false, msg:e.message}); }
});
app.post('/api/admin/order/upload_qrcode', adminAuth, upload.single('qrcode'), async (req, res) => {
    if(req.file) {
       try {
           ...
           notifyAdminUpdate(); res.json({success:true});
       } catch (e) { res.json({success:false, msg: 'Upload failed'}); }
    } else res.json({success:false});
});
app.post('/api/admin/update/announcement', adminAuth, async (req, res) => { await setSetting('announcement', req.body.text); await broadcastGlobalUpdate(); res.json({success:true}); });
app.post('/api/admin/update/popup', adminAuth, async (req, res) => { await setSetting('popup', req.body.open); await broadcastGlobalUpdate(); res.json({success:true}); });
app.post('/api/admin/category/priority', adminAuth, async (req, res) => {
    try { await pool.query('INSERT INTO categories (name, priority) VALUES ($1, $2) ON CONFLICT (name) DO UPDATE SET priority = $2', [req.body.name, parseInt(req.body.priority)]); await broadcastGlobalUpdate(); res.json({success: true}); } catch(e) { res.status(500).json({success: false, msg: e.message}); }
});
...
app.post('/api/admin/confirm_pay', adminAuth, async (req, res) => {
    ...
    try {
        ...
        if (order.status !== '已支付') {
            ...
            } else { try { await handleReferralBonus(order.user_id, parseFloat(order.usdt_amount), '消费'); } catch (bonusErr) {} }
            await client.query('COMMIT'); io.to(`user_${order.user_id}`).emit('order_update'); notifyAdminUpdate(); res.json({success:true});
...
io.on('connection', (socket) => {
    ...
    if (userId) {
        socket.join(userId); socket.userId = userId; onlineUsers.add(userId);
        io.to('admin_room').emit('user_status_change', { userId, online: true });
    }

    socket.on('request_id', (bid, cb) => { if (typeof bid === 'function') { cb = bid; bid = null; } if (typeof cb === 'function') cb(generateShortId()); });

    socket.on('join', async ({ userId, isAdmin, bossId }) => {
        if (isAdmin) {
            socket.join('admin_room'); socket.emit('online_users_list', Array.from(onlineUsers));
        } else if (userId) {
            try {
                const existingUser = await prisma.user.findUnique({ where: { id: userId } });
                if (existingUser && existingUser.isBlocked) { socket.emit('force_logout_blocked'); socket.disconnect(true); return; }
                if (!existingUser) {
                    if (bossId && bossId !== 'SystemRestore') {
                        await prisma.user.create({ data: { id: userId, bossId: bossId } }); socket.join(userId);
                    ...
                    } else { socket.emit('force_logout'); return; }
                } else {
                    socket.join(userId);
                    if (bossId && bossId !== 'SystemRestore' && existingUser.bossId !== bossId) await prisma.user.update({ where: { id: userId }, data: { bossId } });
                }
                socket.userId = userId; onlineUsers.add(userId); io.to('admin_room').emit('user_status_change', { userId, online: true });
            } catch(e) {}
        }
    });
    ...
    socket.on('disconnect', async () => {
        if (socket.userId) {
            onlineUsers.delete(socket.userId); socketAutoReplyHistory.delete(socket.id);
    ...
    socket.on('typing', ({ targetId, isTyping }) => {
        if (targetId === 'admin') { if(socket.userId) io.to('admin_room').emit('user_typing', { userId: socket.userId, isTyping }); } 
        else io.to(targetId).emit('display_typing', { isTyping });
    });

    socket.on('mark_read', async ({ userId, isAdmin }) => {
        try {
            if (isAdmin) { await prisma.message.updateMany({ where: { userId, isFromUser: true, status: { not: 'read' } }, data: { status: 'read' } }); io.to(userId).emit('messages_read_update'); io.to('admin_room').emit('admin_messages_read_sync', { userId }); } 
            else { await prisma.message.updateMany({ where: { userId, isFromUser: false, status: { not: 'read' } }, data: { status: 'read' } }); io.to('admin_room').emit('admin_messages_read', { userId }); }
        } catch(e) {}
    });
    ...
    socket.on('send_message', async (data) => {
        ...
        try {
            if (!onlineUsers.has(userId)) { onlineUsers.add(userId); io.to('admin_room').emit('user_status_change', { userId, online: true }); }
            const user = await prisma.user.findUnique({ where: { id: userId } });
            if (!user || user.isBlocked) { socket.emit('force_logout_blocked'); socket.disconnect(true); return; }
            if (bossId && bossId !== '未知' && user.bossId !== bossId) await prisma.user.update({ where: { id: userId }, data: { bossId } });
            ...
            if (!isCambodiaWorkingTime()) {
                if (!socketAutoReplyHistory.has(socket.id)) {
                    ...
                    setTimeout(() => { socket.emit('receive_message', autoReply); io.to('admin_room').emit('admin_receive_message', { ...autoReply, bossId: 'System_Auto', isMuted: user.isMuted }); }, 1000);
            ...
            if (bot && !user.isMuted && ALLOWED_GROUP_ID) {
                ...
                if (!conf || conf.value === 'on') {
                    try { await bot.telegram.sendMessage(ALLOWED_GROUP_ID, `${(bossId && bossId!=='未知') ? `@${bossId.replace('@','')}` : ''} 🔔 **新消息**\nID: \`${userId}\`\n内容: ${finalType === 'image' ? "📷 [图片]" : content.substring(0, 100)}`, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback(`🗑 删除`, `del_${userId}`)]]) }); } catch(e) {}
                }
            }
        } catch(e) {}
    });
    ...
    socket.on('admin_reply', async ({ targetUserId, content, type, tempId }) => {
        try {
            ...
            if (process.env.VAPID_PUBLIC_KEY) {
                ...
                subs.forEach(sub => { webpush.sendNotification(sub.keys ? { endpoint: sub.endpoint, keys: sub.keys } : sub.endpoint, payload).catch(error => { if (error.statusCode === 404 || error.statusCode === 410) prisma.pushSubscription.delete({ where: { id: sub.id } }).catch(()=>{}); }); });
            }
        } catch(e) {}
    });

    socket.on('admin_toggle_mute', async ({ userId, isMuted }) => { try { await prisma.user.update({ where: { id: userId }, data: { isMuted } }); io.to('admin_room').emit('user_status_update', { userId, isMuted }); } catch(e) {} });
    socket.on('admin_delete_message', async ({ messageId, userId }) => { try { await prisma.message.delete({ where: { id: !isNaN(Number(messageId)) ? Number(messageId) : messageId } }); io.to('admin_room').emit('message_deleted', { messageId, userId }); io.to(userId).emit('message_deleted', { messageId }); } catch(e) { } });
    socket.on('admin_clear_user_data', async ({ userId }) => { try { await prisma.message.deleteMany({ where: { userId } }); await prisma.user.delete({ where: { id: userId } }); await forceDisconnectUser(userId); io.emit('admin_user_deleted', userId); } catch(e) {} });
    socket.on('admin_block_user', async ({ userId }) => {
        try {
            await prisma.message.deleteMany({ where: { userId } }); await prisma.pushSubscription.deleteMany({ where: { userId } }); await prisma.user.delete({ where: { id: userId } });
            const sockets = await io.in(userId).fetchSockets(); sockets.forEach(s => { s.emit('force_logout_blocked'); s.disconnect(true); });
            io.emit('admin_user_blocked', userId); onlineUsers.delete(userId); io.to('admin_room').emit('user_status_change', { userId, online: false });
        } catch(e) { }
    });
    socket.on('admin_merge_user', async ({ oldId, newId }) => {
        try {
            const oldUser = await prisma.user.findUnique({ where: { id: oldId } });
            if (!oldUser) { socket.emit('merge_result', { success: false, msg: `❌ 找不到旧账号: ${oldId}` }); return; }
            await prisma.message.updateMany({ where: { userId: oldId }, data: { userId: newId } }); await prisma.pushSubscription.updateMany({ where: { userId: oldId }, data: { userId: newId } }); await prisma.user.delete({ where: { id: oldId } });
            socket.emit('merge_result', { success: true, msg: `✅ 合并成功` }); io.to('admin_room').emit('admin_user_deleted', oldId); io.to(newId).emit('messages_read_update'); 
        } catch (e) { socket.emit('merge_result', { success: false, msg: `❌ 系统错误` }); }
    });
});
