                ...
                if (text.startsWith('打款 ')) {
                    ...
                    if (member.status === 'left' || member.status === 'kicked') {
                        ...
                        if (member.status !== 'administrator' && member.status !== 'creator') {
                            ...
                            const updatedMember = await bot.telegram.getChatMember(ctx.chat.id, target.userId);
                            if (updatedMember.status === 'restricted' && updatedMember.can_send_messages !== true) {
                                throw new Error('Telegram 未实际恢复该用户的发言权限');
                            }
                        }
                    } catch (e) {
                        console.error("解除用户禁言失败:", e.message);
                        return ctx.reply(`❌ 授权失败，Telegram 未能解除该用户禁言。\n错误：${escapeHTML(e.message)}`, { parse_mode: 'HTML' });
                    }
    ...
    if (finalUSDT <= 0) {
           ...
           try {
                const gatewayParams = {
                    ...
                    return_url: `https://${sourceDomain}/?paid=1`,
                    name: prodName,
                    money: cnyAmount,
        ...
        if (req.body.method === 'USDT' || req.body.method === 'usdt') {
            ...
            try {
                const gatewayParams = {
                    ...
                    out_trade_no: orderId,
                    notify_url: `${process.env.RENDER_EXTERNAL_URL}/api/pay/notify`,
                    return_url: `https://${req.body.source || user.source || 'xaw888.com'}/?paid=1`,
                    name: '余额充值',
                    money: cnyAmount,
                    timestamp: Math.floor(Date.now() / 1000).toString(),
                    sign_type: 'RSA'
